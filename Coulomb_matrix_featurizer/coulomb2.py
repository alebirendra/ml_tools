...
def read_structure(file_path):
    """
    Read a crystal structure from a POSCAR or CIF file using pymatgen or ASE.

    Parameters:
    file_path (str): Path to the POSCAR or CIF file.

    Returns:
    tuple: atomic_numbers (list), positions (list)
    """
    if file_path.endswith('.cif'):
        structure = Structure.from_file(file_path)
    else:  # Assume it's a POSCAR file
...
if __name__ == "__main__":
    # Path to the POSCAR or CIF file
    file_path = 'path/to/your/POSCAR_or_CIF_file'
    
    # Read atomic numbers and positions from the file
    atomic_numbers, positions = read_structure(file_path)
    ...
    features = get_features_from_coulomb_matrix(coulomb_matrix, num_eigenvalues=10)
